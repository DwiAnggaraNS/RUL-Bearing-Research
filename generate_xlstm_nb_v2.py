...
# Intro
cells.append(nbf.v4.new_markdown_cell('# IMPLEMENTATION OF xLSTM ARCHITECTURE ON XJTU DATASET\n\nThis notebook demonstrates the training and validation of the xLSTM-Transformer architecture across multiple window sizes on the XJTU-SY bearing dataset. It strictly adheres to best practices, ensuring clarity, optimized performance, and reproducible results.\n\nAuthor: Anonymous'))
...
# Conf
cells.append(nbf.v4.new_markdown_cell('## 1. Global Configurations\nCentralized hyperparameters, paths, and environment settings. These define the parameters for the xLSTM Transformer training pipeline.'))
cells.append(nbf.v4.new_code_cell(r'''# Dataset Paths
...
cells.append(nbf.v4.new_code_cell('''def calculate_rmse(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    return np.sqrt(mean_squared_error(y_true, y_pred))

def calculate_relative_prediction_error(y_true: np.ndarray, y_pred: np.ndarray) -> float:
...
def scale_health_index_to_rul(hi: np.ndarray, max_rul: float = BEARING_LIFESPAN_TIME) -> np.ndarray:
    return np.clip(hi, 0, 1) * max_rul

def compute_asymmetric_loss(y_pred: np.ndarray, y_true: np.ndarray, a: float = 10.0, b: float = 13.0) -> float:
...
def visualize_predictions(y_true: np.ndarray, y_pred: np.ndarray, title: str, save_path: str = None):
...
# Training loop
cells.append(nbf.v4.new_markdown_cell('## 3. Training & Validation Pipeline\nIterates over the defined validation window sizes, training the xLSTM architecture and extracting test evaluation markers.'))
...
for window_size in WINDOW_SIZES:
    ...
    print(f"[INFO] Loading robust memory payload: {training_filepath}")
    ...
    print("[INFO] Starting Deep Learning Training Phase...")
    model.train()
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_input, batch_target in training_loader:
            ...
            # Critical Stability Control: Gradient Clipping
    ...
    with torch.no_grad():
        for eval_file_path in evaluation_files:
            ...
            x_eval_resistor = x_eval_flat.reshape(num_eval_samples, window_size, num_eval_features)
            tensor_x_eval = torch.tensor(x_eval_resistor, dtype=torch.float32).to(DEVICE)
            ...
            del df_eval, x_eval_flat, x_eval_resistor, tensor_x_eval
            gc.collect()
...
# Final analytics 
cells.append(nbf.v4.new_markdown_cell('## 4. Window Size Performance Evaluation\nAggregates evaluations across all cross-domain setups to discover the optimal structural modeling timeframe.'))
...
if not summary_dashboard.empty:
    ...
    plt.title('Validation Root Mean Square Error vs Target Window Size', fontweight='bold', pad=15)
...
with open(r'd:\Proyek Dosen\Riset Bearing\Notebook-Github\Cross-Domain Generalization RUL Bearing with XAI\Training & Val (XJTU)\3_TRAINING_XLSTM.ipynb', 'w', encoding='utf-8') as f:
    nbf.write(nb, f)

print('Success: Updated Notebook Generated')